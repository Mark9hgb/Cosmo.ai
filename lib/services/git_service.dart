...
class GitServiceState extends Equatable {
  ...
  List<Object?> get props =>
      [repositories, operations, currentRepository, isLoading, error];
}

class GitService extends StateNotifier<GitServiceState> {
  final TerminalService _terminal;

  GitService(this._terminal) : super(const GitServiceState()) {
    _loadSavedRepos();
  }
  ...
  Future<GitRepository?> initRepository(String path,
  ...
  Future<GitOperation> commit(String message, {String? author}) async {
    ...
    final cmd = author != null
        ? 'cd "${repo.path}" && git -c user.name="$author" commit -m "$message"'
        : 'cd "${repo.path}" && git commit -m "$message"';
  ...
  Future<GitOperation> push({String? remote, String? branch}) async {
    ...
    final cmd =
        'cd "${repo.path}" && git push ${remote ?? "origin"} ${branch ?? repo.currentBranch}';
  ...
  Future<GitOperation> pull({String? remote, String? branch}) async {
    ...
    final cmd =
        'cd "${repo.path}" && git pull ${remote ?? "origin"} ${branch ?? repo.currentBranch}';
  ...
  Future<GitOperation> checkout(String branchName,
      {bool createNew = false}) async {
    final repo = state.currentRepository;
    if (repo == null) throw Exception('No repository selected');
  ...
  Future<List<GitCommit>> getCommitHistory(String path,
      {int limit = 50}) async {
    try {
      final cmd =
          '''cd "$path" && git log --pretty=format:"%H|%s|%an|%ae|%ai" -n $limit''';
      final output = await _terminal.executeCommand(cmd);
  ...
  Future<String> getDiff(String path,
      {String? from, String? to, String? file}) async {
    try {
  ...
  Future<GitOperation> stash({String? message, bool apply = false}) async {
    ...
    final stashCmd = apply
        ? 'git stash pop'
        : (message != null ? 'git stash push -m "$message"' : 'git stash');
  ...
  Future<void> refreshCurrentRepository() async {
    ...
    try {
      ...
      final commits =
          await getCommitHistory(state.currentRepository!.path, limit: 1);
      ...
      final updated = GitRepository(
        ...
        currentBranch: branches
            .firstWhere((b) => b.isCurrent,
                orElse: () => GitBranch(name: 'main'))
            .name,
  ...
  void removeRepository(String path) {
    ...
    state = state.copyWith(
      repositories: repos,
      currentRepository: state.currentRepository?.path == path
          ? null
...
final gitServiceProvider =
    StateNotifierProvider<GitService, GitServiceState>((ref) {
  final terminal = TerminalService.instance;
  return GitService(terminal);
});
