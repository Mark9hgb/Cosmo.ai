...
class ProjectService {
  ...
  Future<String> exportSession(ChatSession session) async {
    ...
    final fileName =
        '${_exportPrefix}${session.name.replaceAll(' ', '_')}_$timestamp.zip';
  ...
  Future<String> exportProject(Project project) async {
    ...
    for (final filePath in project.filePaths) {
      try {
        ...
        if (await file.exists()) {
          final bytes = await file.readAsBytes();
          final relativePath =
              filePath.replaceFirst('${project.rootPath}/', '');
    ...
    final timestamp = DateFormat('yyyyMMdd_HHmmss').format(DateTime.now());
    final fileName =
        '${_exportPrefix}${project.name.replaceAll(' ', '_')}_$timestamp.zip';
    final exportPath = '${tempDir.path}/$fileName';
  ...
  Future<Project?> importProject(
      String filePath, String targetDirectory) async {
    try {
      final file = File(filePath);
      final bytes = await file.readAsBytes();
...
class ExportService {
  ...
  static String toHtml(ChatSession session) {
    ...
    buffer.writeln(
        '.user { background: #e3f2fd; padding: 15px; border-radius: 10px; }');
    buffer.writeln(
        '.assistant { background: #f5f5f5; padding: 15px; border-radius: 10px; }');
    buffer.writeln(
        'pre { background: #263238; color: #aed581; padding: 15px; border-radius: 5px; overflow-x: auto; }');
    buffer.writeln('</style></head><body>');
    buffer.writeln('<h1>${session.name}</h1>');
    buffer.writeln(
        '<p><em>Created: ${session.createdAt.toIso8601String()}</em></p>');
      ...
      if (message.commandBlocks != null) {
        for (final cmd in message.commandBlocks!) {
          ...
          if (cmd.result != null) {
            ...
            buffer.writeln(
                '<pre><code>${_escapeHtml(cmd.result!.output)}</code></pre>');
          }
        }
      }
